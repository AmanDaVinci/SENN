...
class SENND(nn.Module):
    ...
    SENND is an extension of the Self-Explaining Neural Network proposed by :
    (https://papers.nips.cc/paper/8003-towards-robust-interpretability-with-self-explaining-neural-networks)
    
    SENND incorporates a constrained variational inference framework
    on a SENN Concept Encoder to learn disentangled representations
    of the basis concepts. The basis concepts are then independently
    sensitive to single generative factors leading to better
    interpretability and lesser overlap with other basis concepts. 
    Such a strong constraint better fulfills the "diversity" 
    ...
    def forward(self, x):
        ...
        explanations : tuple
            Explanation give by the model as a tuple:
